...
class LssBevModel(nn.Module):
    ...
    def get_geometry(self, intrinsics, extrinsics):
        # ensure computations happen on the same device as intrinsics
        device = intrinsics.device
        extrinsics = torch.inverse(extrinsics).to(device)
        rotation, translation = extrinsics[..., :3, :3], extrinsics[..., :3, 3]
        b, n, _ = translation.shape

        # ensure frustum is on the same device as intrinsics to avoid device mismatch
        frustum = self.frustum.to(device)
        points = frustum.unsqueeze(0).unsqueeze(0).unsqueeze(-1)
        points = torch.cat((points[:, :, :, :, :, :2] * points[:, :, :, :, :, 2:3],
                            points[:, :, :, :, :, 2:3]), 5)
        combine_transform = rotation.matmul(torch.inverse(intrinsics).to(device))
    ...
    def encoder_forward(self, images):
        # support both shapes: (B, n, c, H, W) or (B, c_total, H, W) where c_total = n*c
        if images.dim() == 5:
            b, n, c, h, w = images.shape
            images = images.view(b * n, c, h, w)
        elif images.dim() == 4:
            b, c_total, h, w = images.shape
            # try infer per-camera channels (commonly 3), compute n
            # fallback: assume 3 channels per camera
            c = 3
            if c_total % c != 0:
                # fallback: set n=1, c=c_total
                n = 1
                c = c_total
            else:
                n = c_total // c
