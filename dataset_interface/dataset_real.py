...
class ParkingDataModuleReal(torch.utils.data.Dataset):
    def __init__(self, config: Configuration, is_train):
        ...
        self.target_point = []
        # history trajectories per sample (list of list of [x,y])
        self.history = []
        self.create_gt_data()
    ...
    def __getitem__(self, index):
        ...
        data['history_traj'] = torch.from_numpy(self.history[index]) if len(self.history) > 0 else None
    ...
    def create_gt_data(self):
        ...
        for task_index, task_path in tqdm.tqdm(enumerate(all_tasks)):  # task iteration
            ...
            for ego_index in range(0, traje_info_obj.total_frames):  # ego iteration
                ...
                history_pts = []
                hist_len = int(getattr(self.cfg, 'history_len', getattr(self.cfg, 'hist_frame_nums', 10)))
                # historical frames: ego_index - hist_len ... ego_index-1
                start_h = max(0, ego_index - hist_len)
                for h_idx in range(start_h, ego_index):
                    hist_pose = traje_info_obj.get_trajectory_point(h_idx)
                    hist_pose_in_ego = hist_pose.get_pose_in_ego(world2ego_mat)
                    history_pts.append([hist_pose_in_ego.x, hist_pose_in_ego.y])
                # if fewer than hist_len, pad at front with zeros so most recent entries at end
                if len(history_pts) < hist_len:
                    pad_n = hist_len - len(history_pts)
                    history_pts = [[0.0, 0.0]] * pad_n + history_pts
    ...
    def create_predict_point_gt(self, traje_info_obj: TrajectoryInfoParser, ego_index: int, world2ego_mat: np.ndarray) -> Tuple[List[int], List[float]]:
    ...
    def create_parking_goal_gt(self, traje_info_obj: TrajectoryInfoParser, world2ego_mat: np.ndarray) -> Tuple[List[float], List[float]]:
    ...
    def format_transform(self):
        ...
        # history trajectories: (N, hist_len, 2)
        try:
            self.history = np.array(self.history).astype(np.float32)
        except Exception:
            self.history = np.zeros((len(self.traj_point), int(getattr(self.cfg, 'history_len', getattr(self.cfg, 'hist_frame_nums', 10))), 2), dtype=np.float32)
