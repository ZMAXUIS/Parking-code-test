...
import sys
import pathlib
sys.path.insert(0,'/home/hfut/zd/parkingtest/e2e-parking-carla-master/CARLA_0.9.11/carla/PythonAPI/carla/dist/carla-0.9.11-py3.7-linux-x86_64.egg')
...
## ❌ 世界每帧被 tick 两次  而 CARLA 同步模式的关键要求是： 每一帧必须严格只 tick 一次，否则传感器全部 miss 掉回调
    def tick(self, clock):
        ...
        # carla 0.9.15
        #is_collision = self._world.on_tick(clock, self._parking_goal_index)
        if is_collision:
    ...
    def start_next_parking(self):
        ...
        # 清理之前任务的轨迹缓存，确保每次评估任务的轨迹独立
        try:
            self._world.clear_trajectory()
        except Exception:
            # 向后兼容：如果 World 没有实现 clear_trajectory，不报错
            logging.debug('World.clear_trajectory not available or failed')
    ...
    def save_mean_std_csv(self):
        ...
        for i in range(self._eva_task_nums):
            ...
            # 仅对数值列做统计，避免字符串列导致问题
            numeric_df = df_row_i.select_dtypes(include="number")
            if numeric_df.empty:
                row_i_mean = df_row_i.mean(axis=0).to_frame().T
                row_i_std = (df_row_i.std(axis=0, ddof=0).to_frame().T / math.sqrt(max(1, self._eva_parking_nums)))
            else:
                row_i_mean = numeric_df.mean(axis=0).to_frame().T
                # 使用实际的停车样本数做归一化，避免硬编码常数
                row_i_std = (numeric_df.std(axis=0, ddof=0).to_frame().T / math.sqrt(max(1, self._eva_parking_nums)))
    ...
    # def world_tick(self, clock):
    #     return self._world.tick(clock, self._parking_goal_index)
