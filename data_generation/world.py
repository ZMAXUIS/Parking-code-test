...
TRAJECTORY_RECORDING_DEFAULT = False
TRAJECTORY_SAVE_DEFAULT = False
TRAJECTORY_SAVE_DIR = os.path.join('log', 'trajectories')
# ----------------------------------------------------------------
...
class World(object):
    def __init__(self, carla_world, args):
        ...
        # read trajectory recording flag from args if provided, otherwise use default
        self._record_trajectory = getattr(args, 'record_trajectory', TRAJECTORY_RECORDING_DEFAULT)

        self._save_trajectory = getattr(args, 'save_trajectory', TRAJECTORY_SAVE_DEFAULT)
        self._trajectory_save_dir = getattr(args, 'trajectory_save_dir', TRAJECTORY_SAVE_DIR)
        if self._save_trajectory:
            try:
                os.makedirs(self._trajectory_save_dir, exist_ok=True)
            except Exception:
                logging.warning('Could not create trajectory save directory: %s', self._trajectory_save_dir)
        ...
        # trajectory buffer (only present when recording enabled)
    ...
    def restart(self, seed, target_index, ego_transform):
        ...
        # clear trajectory for new run if enabled
        if self._record_trajectory:
    ...
    def tick(self, clock, target_index):
        ...
        # if trajectory recording enabled, append current transform and visualize stored points
        if self._record_trajectory:
            # store (x,y,z) for later use (visualization compatibility)
            self.trajectory_points.append((t.x, t.y, t.z))

            try:
                # use real wall-clock time as timestamp (ISO format)
                ts = datetime.datetime.now().isoformat()
                yaw = float(t.rotation.yaw) if hasattr(t, 'rotation') else 0.0
                v = self._player.get_velocity()
                speed = float((v.x ** 2 + v.y ** 2 + v.z ** 2) ** 0.5) if v is not None else 0.0
                record = {'time': ts, 'x': float(t.x), 'y': float(t.y), 'z': float(t.z), 'yaw': yaw, 'speed': speed}
                # keep a separate records list for saving richer info
                if not hasattr(self, 'trajectory_records'):
                    self.trajectory_records = []
                self.trajectory_records.append(record)
            except Exception:
                # 不要因为记录失败影响主循环
                logging.debug('Failed to append detailed trajectory record')

            # draw each point as a small debug point in the world
    ...
    def destroy(self):
        ...
        # 如果开启了保存轨迹选项，则在销毁前保存当前轨迹（如果存在）
        if getattr(self, '_save_trajectory', False) and getattr(self, 'trajectory_points', None):
            try:
                saved_path = self.save_trajectory()
                logging.info('Saved trajectory to %s', saved_path)
            except Exception as e:
                logging.warning('Failed to save trajectory: %s', e)
    ...
    def clear_trajectory(self):
        if hasattr(self, 'trajectory_points'):
            self.trajectory_points = []
        if hasattr(self, 'trajectory_records'):
            self.trajectory_records = []


    def save_trajectory(self, filename: str = None):

        records = getattr(self, 'trajectory_records', None)
        points = getattr(self, 'trajectory_points', None)
        if not records and not points:
            return None
        os.makedirs(self._trajectory_save_dir, exist_ok=True)
        if filename is None:
            ts = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f'trajectory_{ts}.json'
        path = os.path.join(self._trajectory_save_dir, filename)
        try:
            if records:
                to_write = records
            else:
                # fallback: convert points tuples to simple dicts
                to_write = [{'x': float(p[0]), 'y': float(p[1]), 'z': float(p[2])} for p in points]
            with open(path, 'w') as f:
                json.dump(to_write, f, indent=2)
            return path
        except Exception as e:
            logging.warning('Failed to write trajectory file %s: %s', path, e)
            raise
