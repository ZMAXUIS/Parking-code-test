...
class ParkingInferenceModuleReal:
    def __init__(self, inference_cfg: InferenceConfiguration, ros_interface_obj: RosInterface, eval_bag_path: str = None, eval_out_dir: str = None):
    ...
    def load_model(self, parking_pth_path):
        ...
        try:
            ...
            for k, v in sd.items():
                ...
                new_sd[new_k] = v
            # attempt to load state dict into model with strict=False
            try:
                self.model.load_state_dict(new_sd, strict=False)
            except Exception:
                # fallback: filter keys present in model
            ...
            print(f"[Inference] Failed to load model from {parking_pth_path}: {e}\n{tb}")
            raise
