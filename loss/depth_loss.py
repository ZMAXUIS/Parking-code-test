...
class DepthLoss(nn.Module):
    def __init__(self, cfg: Configuration):
        ...
        # Ensure d_bound values are numeric
        d_bound = [float(x) for x in self.cfg.d_bound]
        self.d_bound = d_bound
        self.down_sample_factor = self.cfg.bev_down_sample
        self.depth_channels = int((d_bound[1] - d_bound[0]) / d_bound[2])
    ...
    def forward(self, depth_preds, depth_labels):
        ...
        with autocast(enabled=False):
            # Ensure no NaN or Inf in predictions and labels
            depth_preds = torch.nan_to_num(depth_preds, nan=0.0, posinf=1e5, neginf=-1e5)
            depth_labels = torch.nan_to_num(depth_labels, nan=0.0, posinf=1e5, neginf=-1e5)
